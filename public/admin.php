...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
    ...
    if ($action === 'login') {
        if (!rate_limit($db, 'admin_login:' . $ip, 5, 900)) {
            fail_json('Pøíliš mnoho pokusù.', 429);
        }
...
<body data-page="admin" data-week-start="<?= h($weekStart->format('Y-m-d')) ?>" data-week-label="<?= h($weekLabel) ?>" data-grid-start="<?= h(cfg('grid_start')) ?>" data-grid-end="<?= h(cfg('grid_end')) ?>" data-step-min="<?= h((string) cfg('grid_step_min')) ?>" data-space-label-a="<?= h((string) cfg('space_label_a')) ?>" data-space-label-b="<?= h((string) cfg('space_label_b')) ?>">
  <div class="layout">
    <header class="topbar">
      ...
      <div class="actions">
        <a class="btn ghost" href="/">Zpìt na rozpis</a>
        <?php if ($admin): ?>
        ...
        <div class="panel narrow">
          ...
          <form id="form-login" class="form">
            ...
            <button class="btn primary" type="submit">
              <span class="btn-text">Pøihlásit</span>
        ...
        <div class="grid-admin">
          ...
          <section class="panel">
            ...
            <form id="form-recurring" class="form">
              ...
              <div class="grid-2">
                ...
                <label>
                  ...
                  <select name="dow" required>
                    <option value="1">Pondìlí</option>
                    <option value="2">Úterý</option>
                    <option value="3">Støeda</option>
                    <option value="4">Ètvrtek</option>
                    <option value="5">Pátek</option>
                    <option value="6">Sobota</option>
                    <option value="7">Nedìle</option>
                  </select>
                </label>
                <label>
                  Prostor
              ...
              <button class="btn primary" type="submit">
                <span class="btn-text">Vytvoøit</span>
                <span class="spinner" aria-hidden="true"></span>
              </button>
            </form>
          </section>
